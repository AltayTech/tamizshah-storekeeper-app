...
import '../models/request/delivery_main.dart';
...
class Deliveries with ChangeNotifier {
  ...
  DeliveryWasteItem _deliveryWasteItem;
  ...
  Future<void> sendRequest(RequestWaste request, bool isLogin, int id) async {
    ...
    try {
      if (isLogin) {
        ...
        final url = Urls.rootUrl + Urls.deliveriesEndPoint + '/$id';
        print('url  $url');
...
//  }
  ...
  Future<void> retrieveDeliveryItem(int collectId) async {
    print('retrieveDeliveryItem');
    ...
    try {
      ...
      _deliveryWasteItem = deliveryWasteItem;
    } catch (error) {
      print(error.toString());
      throw (error);
    }
  ...
  DeliveryWasteItem get deliveriesWasteItem => _deliveryWasteItem;
